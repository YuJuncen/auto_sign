...
class CSUSTServer(SignServer):
    ...
    def sign(self, info):
        ...
        self.throw_exception_with(f"缺少了必要的信息：{set(fields) - info.get('data', set())}", info)
    ...
    def dosign(self, info):
        ...
        if response_json.get("data", {}).get("isSuccess") != "1":
            ...
            self.throw_exception_with(f"签到失败，原因是：{response_json.get('message') or '不知道。'}", response_json)
